...
![MySQL](images/0101.MySQL_Architecture.png)
...
数据更新流程同查询流程相似，走的还是 `连接器 - 缓存 - 分析器 - 优化器 - 执行器`，不同点是执行更新 SQL 时会将缓存中所有与被更新表的缓存失效。

相较于查询流程更新流程还涉及到两个模块：RedoLog 模块和 BinLog 模块。

### redo log 模块

关键词：InnoDB 引擎独有

当我们执行一条更新 SQL 时，会先从数据库中查询到要修改的数据，修改后再写入到数据库中。因此当更新操作频繁时，查询操作会消耗很多的IO资源，`查询-修改-入库` 的操作并不高效。

为了提升更新效率，InnoDB 引擎在执行更新操作时会先把记录写到 `redo log` 中并更新内存，此时更新操作就执行完毕了。当 InnoDB 在空闲的时候会将操作记录更新到磁盘中。

问题：RedoLog 到底记录的什么？更新内存时更新的什么？

![CrashSafe](images/0102.crash-safe.png)

RedoLog 是固定大小的，例如配置一组四个文件，每个文件 1GB 大小，那么总共可以记录 4GB 大小的操作。`操作` 的写入是循环的，通过 `write pos` 和 `check point` 两个标记位实现。`write pos` 表示当前记录的位置，`check point` 表示要擦除的位置，`write pos` 和 `check point` 之间的空间是可写入部分。

RedoLog 这种方式叫 `crash-safe`

### BinLog 模块

特点：

1. MySQL Service 层实现的，所有引擎共享。

因为MySQL之前默认的执行引擎是MyISAM，没有 crash-safe 的能力，BinLog 日志只能用于归档，所以InnoDB采用另外一套日志系统来实现 crash-safe。
