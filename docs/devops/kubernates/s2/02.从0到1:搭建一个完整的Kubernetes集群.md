...
参考文章:[2021 年 Kubernetes(k8s)服务集群安装部署](https://zhuanlan.zhihu.com/p/340575279)
...
升级 Linux 内核为 4.44 版本(亲测这一步跳过不影响)
...
1、安装 kubernetes 的时候，需要安装 kubelet, kubeadm 等包，但 k8s 官网给的 yum 源是http://packages.cloud.google.com，国内访问不了，此时我们可以使用阿里云的yum仓库镜像。
